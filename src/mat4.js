var geom = (function(geom){
	var Mat4 = function() {
		var m = this.data = new Float32Array(16);
		m[0] = m[5] = m[10] = m[15] = 1;
		m[1] = m[2] = m[3] = m[4] = m[6] = m[7] = m[8] = m[9] = m[11] = m[12] = m[13] = m[14] = 0;
	};
	Mat4.prototype.perspective = function(fov, aspect, near, far) {
		var m = this.data, f = Math.tan(Math.PI * 0.5 - 0.5 * fov), range = 1.0 / (near - far);
		m[0] = f / aspect; 
		m[5] = f; 
		m[10] = (near + far) * range;
		m[11] = -1;
	...
	Mat4.prototype.lockAt = function(x, y, z) {
	...
	Mat4.prototype.rotateX = function(a) {
	...
	Mat4.prototype.rotateY = function(a) {
		var m = this.data, c = Math.cos(a), s = Math.sin(a);
	...
	Mat4.prototype.rotateZ = function(a) {
		var m = this.data, c = Math.cos(a), s = Math.sin(a);
	...
	Mat4.prototype.invert = function() {
		...
		var s = 1 / (m00 * t0 + m10 * t1 + m20 * t2 + m30 * t3);
		m[0] = t0 * s; 
		m[1] = t1 * s; 
		m[2] = t2 * s; 
		m[3] = t3 * s;
		m[4] = ((n1 * m10 + n2 * m20 + n5 * m30) - (n0 * m10 + n3 * m20 + n4 * m30)) * s;
		m[5] = ((n0 * m00 + n7 * m20 + n8 * m30) - (n1 * m00 + n6 * m20 + n9 * m30)) * s;
		m[6] = ((n3 * m00 + n6 * m10 + n11 * m30) - (n2 * m00 + n7 * m10 + n10 * m30)) * s;
		m[7] = ((n4 * m00 + n9 * m10 + n10 * m20) - (n5 * m00 + n8 * m10 + n11 * m20)) * s;
		m[8] = ((n12 * m13 + n15 * m23 + n16 * m33) - (n13 * m13 + n14 * m23 + n17 * m33)) * s;
		m[9] = ((n13 * m03 + n18 * m23 + n21 * m33) - (n12 * m03 + n19 * m23 + n20 * m33)) * s;
		m[10] = ((n14 * m03 + n19 * m13 + n22 * m33) - (n15 * m03 + n18 * m13 + n23 * m33)) * s;
		m[11] = ((n17 * m03 + n20 * m13 + n23 * m23) - (n16 * m03 + n21 * m13 + n22 * m23)) * s;
		m[12] = ((n14 * m22 + n17 * m32 + n13 * m12) - (n16 * m32 + n12 * m12 + n15 * m22)) * s;
		m[13] = ((n20 * m32 + n12 * m02 + n19 * m22) - (n18 * m22 + n21 * m32 + n13 * m02)) * s;
		m[14] = ((n18 * m12 + n23 * m32 + n15 * m02) - (n22 * m32 + n14 * m02 + n19 * m12)) * s;
		m[15] = ((n22 * m22 + n16 * m02 + n21 * m12) - (n20 * m12 + n23 * m22 + n17 * m02)) * s;
	};
	Mat4.prototype.concat = function(that) {
	...
	Mat4.prototype.transform = function(source, sourceOffset, output, outputOffset) {
		...
		for(var i = 0; i<source.length; i++) {
			...
			var z = source.get(i, sourceOffset + 2);
			output.set(i, outputOffset, m[0] * x + m[4] * y + m[8] * z + m[12]);
			output.set(i, outputOffset + 1, m[1] * x + m[5] * y + m[9] * z + m[13]);
			output.set(i, outputOffset + 2, m[2] * x + m[6] * y + m[10] * z + m[14]);
		}
	};
	geom.mat4 = function() {
		return new Mat4();
